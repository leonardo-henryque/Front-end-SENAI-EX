...
function renderProdutos(products) {
    ...
    <div class="col-md-4">
        <div class="card">
            <img src="${p.thumbnail}" alt="${p.title}" class="card-img" height="180" width="150">
            <div class="caracteristicas_produtos">
                <strong>${p.title}</strong><br>
                <em>${p.category}</em><br>
                Avaliação: ${p.rating} | Estoque: ${p.stock}
            </div>
            <p><strong>Preço: R$ ${p.price}</strong></p>
        </div>
    </div>
  `).join('');
}
...
async function getProdutos() {
    ...
    try {
        ...
        renderProdutos(data.products); // Corrigido para usar `data.products`
    } catch (err) {
        showError(err.message ?? 'Falha ao buscar dados');
    }
}
